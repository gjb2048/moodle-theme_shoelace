...
/**
 * Core renderere unit tests for the Eyelet theme.
 * @group theme_eyelet
 */
class theme_eyelet_corerenderer_testcase extends advanced_testcase {

    protected $outputus;

    protected function setUp() {
        global $PAGE;
        $this->outputus = $PAGE->get_renderer('theme_eyelet', 'core');
        \theme_shoelace\toolbox::set_core_renderer($this->outputus);
    }

    public function test_version() {
        $ourversion = \theme_shoelace\toolbox::get_setting('version');
        $coretheme = \theme_config::load('eyelet');
    ...
    public function test_textcolour() {
        ...
        $this->assertEquals('#653cae', $ourcolour);
    }
}